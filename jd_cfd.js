...
!(async () => {
  ...
  let res = await getAuthorShareCode('127.0.0.1')
  if (!res) {
    $.http.get({url: '127.0.0.1'}).then((resp) => {}).catch((e) => console.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await getAuthorShareCode('127.0.0.1')
  }
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `127.0.0.1`, timeout: 30 * 1000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))
...
function uploadShareCode(code) {
  return new Promise(async resolve => {
    $.post({url: `127.0.0.1/upload/cfd?code=${code}&ptpin=${encodeURIComponent(encodeURIComponent($.UserName))}`, timeout: 30 * 1000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))
